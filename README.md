...
이 프로젝트는 **2018~2019년에 작성된 코드**를 참고용으로 보존하는 저장소입니다.

- **작성 시기**: 2018년 4월 ~ 2019년 2월
- **마지막 업데이트**: 2019년 2월
- **현재 상태**: 아카이브 (추가 유지보수 예정 없음)

> **주의**: 각 기관의 API 구조, 엔드포인트, 인증 방식 등이 변경되었을 가능성이 높습니다. 코드를 참고용으로만 활용하시고, 실제 사용 시에는 각 기관의 최신 API 문서를 반드시 확인하세요.
...
| 기관 | API 형식 | 작성 당시 상태 | 추천 파일 | 폴더 |
|------|---------|--------------|-----------|------|
| 통계청 (KOSIS) | JSON | 완료 (2018) | `json_engine_api_format_auto(완).sas` | [KOSIS/](./KOSIS/) |
| 열린재정 | JSON | 완료 (2018) | `model_04_fis.sas` | [열린재정/](./열린재정/) |
| 국토교통부 통계누리 | JSON | 기본 (2018) | `model_01.sas` | [국토교통부 통계누리/](./국토교통부%20통계누리/) |
| 환경부 환경통계포털 | XML | 완료 (2018) | `model_03_env_xml.sas` | [환경부 환경통계포털/](./환경부%20환경통계포털/) |
| 지방재정365 | JSON | 완료 (2019) | `01.sas` | [지방재정365/](./지방재정365/) |

> ⚠️ 위 상태는 **작성 당시(2018~2019년)** 기준입니다. 현재 API 작동 여부는 보장되지 않습니다.

---

## 빠른 시작

> ⚠️ **참고**: 아래 내용은 2018~2019년 작성 당시 기준입니다. API 엔드포인트, 파라미터, 인증 방식 등이 변경되었을 수 있습니다.

### 요구사항

- **SAS 9.4** (유니코드 지원 버전, TS1M5 이상 권장)
- 각 기관별 **API 키** (무료 발급) - 발급 페이지 URL이 변경되었을 수 있음
...
> ⚠️ 아래 URL은 2018~2019년 기준이며, 변경되었을 수 있습니다.
